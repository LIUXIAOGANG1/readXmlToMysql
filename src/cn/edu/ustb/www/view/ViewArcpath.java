...
public class ViewArcpath extends JInternalFrame {

	private static final long serialVersionUID = 4050380544993622536L;
	...
	private void LayOut() throws Exception {
		...
		String sql = "select id, arcId, netId, x, y, curvePoint"
				+ " from arcpath where isDel = 0 and netId = " + QueryXml.ID;
	...
	public static void buildTable(String str) {
		...
		String[] name = { "I D", "arcId", "netId", "x", "y", "curvePoint" };
	...
	class jbtExit_actionAdapter implements ActionListener {
		private ViewArcpath adaptee;

		jbtExit_actionAdapter(ViewArcpath adaptee) {
	...
	public void jbtExit_actionPerformed(ActionEvent e) {
		...
		int result = JOptionPane.showOptionDialog(null, "是否退出arcpath查询?",
	...
	class jbtDelete_actionAdapter implements ActionListener {
		...
		jbtDelete_actionAdapter(ViewArcpath adaptee) {
			this.adaptee = adaptee;
		}
	...
	public void jbtDelete_actionPerformed(ActionEvent e) {
		if (jtf1.getText().trim().equals("")) {
			...
			if (result == JOptionPane.NO_OPTION)
				return;
			String sqlDel = "Update arcpath set isDel = 1,destroyer = '"
					+ LoginSystem.user.getText().trim() + "',destroyTime='"
					+ df.format(new Date()) + "' where id = '"
					+ jtf1.getText().trim() + "' and arcId = '"
					+ jtf2.getText().trim() + "' and netId = " + QueryXml.ID;
			...
			if (jdbcAdapter.DeleteObject(sqlDel)) {
				...
				String sqldelete = "select id, arcId, netId, x, y, curvePoint"
						+ " from arcpath where isDel = 0 and netId = "
						+ QueryXml.ID;
	...
	class jtb_mouseAdapter extends MouseAdapter {
		...
		jtb_mouseAdapter(ViewArcpath adaptee) {
			this.adaptee = adaptee;
		}
	...
	public void jtb_mouseClicked(MouseEvent e) {
		...
		ArcId = jtb.getValueAt(selectrow, 1).toString();
		...
		// 填写表最下端的文本框
		sqlStr = "select arcId, netId, x, y, curvePoint"
				+ " from arcpath where isDel = 0 and netId = " + QueryXml.ID
				+ " and id = '" + ID + "' and arcId = '" + ArcId + "'";
	...
	class jbtSave_actionAdapter implements ActionListener {
		...
		jbtSave_actionAdapter(ViewArcpath adaptee) {
			this.adaptee = adaptee;
		}
	...
	public void jbtSave_actionPerformed(ActionEvent e) {
		if (jtf1.getText().trim().equals("")) {
			...
			if (result == JOptionPane.NO_OPTION)
				...
				String sql = "Update arcpath set x = '" + jtf3.getText().trim()
						+ "', y = '" + jtf4.getText().trim()
						+ "', curvePoint = '" + jtf5.getText().trim()
						+ "' where id = '" + jtf1.getText().trim()
						+ "' and arcId = '" + jtf2.getText().trim()
						+ "' and netId = " + QueryXml.ID;
				...
				String sqlsave = "select id, arcId, netId, x, y, curvePoint"
						+ " from arcpath where isDel = 0 and netId = "
						+ QueryXml.ID;
